...
fn setup_schema(database: &Database, request: &OpenDatabaseRequest) -> Result<(), StorageError> {
...
fn map_chunk_record_from_js(value: &JsValue) -> Result<MapChunkRecord, StorageError> {
    let record: MapChunkRecordRecord = from_value(value.clone()).map_err(map_serde_decode_error)?;
    let key = ChunkKey::new(
        WorldId::from(record.world_id),
        ChunkCoord {
            cx: record.cx,
